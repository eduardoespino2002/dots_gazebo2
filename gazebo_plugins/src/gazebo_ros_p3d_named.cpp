...
#include <gazebo_plugins/gazebo_ros_p3d_named.hpp>
...
class GazeboRosP3DNamedPrivate
{
public:
  ...
  /// Name to use for child frame id
...
GazeboRosP3DNamed::GazeboRosP3DNamed()
: impl_(std::make_unique<GazeboRosP3DNamedPrivate>())
{
}
...
// Load the controller
void GazeboRosP3DNamed::Load(gazebo::physics::ModelPtr model, sdf::ElementPtr sdf)
{
  ...
  impl_->child_frame_name_ = link_name;
  if (sdf->HasElement("child_frame")) { 
    impl_->child_frame_name_ = sdf->GetElement("child_frame")->Get<std::string>();
    RCLCPP_WARN(impl_->ros_node_->get_logger(), "Using %s for child_frame_id", 
            impl_->child_frame_name_.c_str());
  }

  // Listen to the update event. This event is broadcast every simulation iteration
  impl_->update_connection_ = gazebo::event::Events::ConnectWorldUpdateBegin(
    std::bind(&GazeboRosP3DNamedPrivate::OnUpdate, impl_.get(), std::placeholders::_1));
}

// Update the controller
void GazeboRosP3DNamedPrivate::OnUpdate(const gazebo::common::UpdateInfo & info)
{
  if (!link_) {
    return;
  }
  ...
  pose_msg.child_frame_id = child_frame_name_;
...
GZ_REGISTER_MODEL_PLUGIN(GazeboRosP3DNamed)
